...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        W = tf.get_variable('W',shape=[self.config.n_features,self.config.n_classes],dtype=tf.float32,initializer=tf.initializers.zeros)
        b = tf.get_variable('b',shape=[1,self.config.n_classes],dtype=tf.float32,initializer=tf.initializers.zeros)
    ...
    def add_training_op(self, loss):
        ...
        """
        ### YOUR CODE HERE
        opti = tf.train.GradientDescentOptimizer(learning_rate=self.config.lr)
