...
class SequencePredictor(Model):
    ...
    def add_training_op(self, loss):
        ...
        if self.config.clip_gradients:
            ...
            grads = [grad for grad,_ in gradients]
            var = [v for _,v in gradients] 
            clipped_grads, _= tf.clip_by_global_norm(grads, self.config.max_grad_norm)
            self.grad_norm = tf.global_norm(clipped_grads)
            train_op = optimizer.apply_gradients(zip(clipped_grads,var))
        else:
            gradients = optimizer.compute_gradients(loss)
            self.grad_norm = tf.global_norm([grad for grad,_ in gradients])
