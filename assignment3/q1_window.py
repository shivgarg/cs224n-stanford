...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        prefix = [start]*window_size
        suffix = [end]*window_size
        sentence = prefix + sentence + suffix
        for i in xrange(window_size,window_size+num_words):
            arr = [val for word in sentence[i-window_size:i+window_size+1] for val in word]
            windowed_data.append((arr,labels[i-window_size]))
		### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        h = tf.layers.dense(x,self.config.hidden_size,activation=tf.nn.relu,kernel_initializer=tf.contrib.layers.xavier_initializer())
        h_drop = tf.nn.dropout(h,dropout_rate)
        pred = tf.layers.dense(h_drop,self.config.n_classes,kernel_initializer=tf.contrib.layers.xavier_initializer())
        ### END YOUR CODE
...
#        b1 = tf.get_variable('b1',shape=[1,self.config.hidden_size],dtype=tf.float32,initializer=tf.initializers.zeros)
...
#        b2 = tf.get_variable('b2',shape=[1,self.config.n_classes],dtype=tf.float32,initializer=tf.initializers.zeros)
