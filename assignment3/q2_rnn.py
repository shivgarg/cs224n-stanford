...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if length < max_length:
            sentence_1 = sentence + [zero_vector]*(max_length-length)
            labels_1 = labels + [zero_label]*(max_length-length)
            mask = [True]*length + [False] *(max_length-length)
            ret.append((sentence_1,labels_1,mask))
        else:
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        b_2 = tf.get_variable('b_2',shape=[1,self.config.n_classes],dtype=tf.float32,initializer=tf.initializers.zeros())
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ### YOUR CODE HERE (~6-10 lines)
                out, h = cell(x[:,time_step,:],h)
                out = tf.matmul(h,U) + b_2
                preds.append(out)
                ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        loss = tf.reduce_mean(tf.multiply(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder,logits=preds),tf.cast(self.mask_placeholder,dtype=tf.float32)))
        ### END YOUR CODE
        return loss
